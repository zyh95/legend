...
$(document).ready(function(){
    document.getElementById("wdr-start-time").valueAsDate=new Date();
    document.getElementById("wdr-end-time").valueAsDate=new Date();
    var targetJson;

    //初始页面
    var mydate=new Date();
    var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
    initAjaxUseGet(path+"warehouseDeliveryRecord","json","application/json;charset=utf-8",".warehouse-delivery-record-table","all",0,today);

    //筛选事件
    $("#wdr-select").on('click',function(){
        var $selectStartTime=DateToUnix($("#wdr-start-time").val());
        var $selectEndTime=DateToUnix($("#wdr-end-time").val());
        if($selectStartTime>$selectEndTime){
            alert("请选择正确的时间");
        }else{
            initAjaxUseGet(path+"warehouseDeliveryRecord","json","application/json;charset=utf-8",".warehouse-delivery-record-table","selectPage",$selectStartTime,$selectEndTime);
        }
    })

    //近三天
    $("#wdr-threeDays").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-3*60*60*24*1000;
        initAjaxUseGet(path+"warehouseDeliveryRecord","json","application/json;charset=utf-8",".warehouse-delivery-record-table","threeDaysPage",otherday,today);
    })

    //近一周
    $("#wdr-aWeek").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-7*60*60*24*1000;
        initAjaxUseGet(path+"warehouseDeliveryRecord","json","application/json;charset=utf-8",".warehouse-delivery-record-table","aWeekPage",otherday,today);
    })

    //近两周
    $("#wdr-twoWeeks").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-14*60*60*24*1000;
        initAjaxUseGet(path+"warehouseDeliveryRecord","json","application/json;charset=utf-8",".warehouse-delivery-record-table","twoWeeksPage",otherday,today);
    })

    //近一个月
    $("#wdr-aMonth").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=DateToUnix(""+mydate.getFullYear()+"-"+mydate.getMonth()+"-"+mydate.getDate())
        initAjaxUseGet(path+"warehouseDeliveryRecord","json","application/json;charset=utf-8",".warehouse-delivery-record-table","aMonthPage",otherday,today);
    })
})
