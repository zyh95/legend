...
$(document).ready(function(){
    document.getElementById("pmir-start-time").valueAsDate=new Date();
    document.getElementById("pmir-end-time").valueAsDate=new Date();
    var targetJson;

    //初始页面
    var mydate=new Date();
    var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
    initAjaxUseGet(path+"ProductModelInfo/getProductModelInfo","json","application/json;charset=utf-8",".product-model-info-record-table","all",0,today);

    //筛选事件
    $("#pmir-select").on('click',function(){
        var $selectStartTime=DateToUnix($("#pmir-start-time").val());
        var $selectEndTime=DateToUnix($("#pmir-end-time").val());
        if($selectStartTime>$selectEndTime){
            alert("请选择正确的时间");
        }else{
            initAjaxUseGet(path+"ProductModelInfo/getProductModelInfo","json","application/json;charset=utf-8",".product-model-info-record-table","selectPage",$selectStartTime,$selectEndTime);
        }
    })

    //近三天
    $("#pmir-threeDays").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-3*60*60*24*1000;
        initAjaxUseGet(path+"ProductModelInfo/getProductModelInfo","json","application/json;charset=utf-8",".product-model-info-record-table","threeDaysPage",otherday,today);
    })

    //近一周
    $("#pmir-aWeek").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-7*60*60*24*1000;
        initAjaxUseGet(path+"ProductModelInfo/getProductModelInfo","json","application/json;charset=utf-8",".product-model-info-record-table","aWeekPage",otherday,today);
    })

    //近两周
    $("#pmir-twoWeeks").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-14*60*60*24*1000;
        initAjaxUseGet(path+"ProductModelInfo/getProductModelInfo","json","application/json;charset=utf-8",".product-model-info-record-table","twoWeeksPage",otherday,today);
    })

    //近一个月
    $("#pmir-aMonth").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=DateToUnix(""+mydate.getFullYear()+"-"+mydate.getMonth()+"-"+mydate.getDate())
        initAjaxUseGet(path+"ProductModelInfo/getProductModelInfo","json","application/json;charset=utf-8",".product-model-info-record-table","aMonthPage",otherday,today);
    })
})
