...
$(document).ready(function(){
    document.getElementById("fosr-start-time").valueAsDate=new Date();
    document.getElementById("fosr-end-time").valueAsDate=new Date();
    var targetJson;

    //初始页面
    var mydate=new Date();
    var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
    initAjaxUseGet(path+"FactoryOutStorageRecord/getFactoryOutStorageRecord","json","application/json;charset=utf-8",".factory-out-storage-record-table","all",0,today);

    //筛选事件
    $("#fosr-select").on('click',function(){
        var $selectStartTime=DateToUnix($("#fosr-start-time").val());
        var $selectEndTime=DateToUnix($("#fosr-end-time").val());
        if($selectStartTime>$selectEndTime){
            alert("请选择正确的时间");
        }else{
            initAjaxUseGet(path+"FactoryOutStorageRecord/getFactoryOutStorageRecord","json","application/json;charset=utf-8",".factory-out-storage-record-table","selectPage",$selectStartTime,$selectEndTime);
        }
    })

    //近三天
    $("#fosr-threeDays").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-3*60*60*24*1000;
        initAjaxUseGet(path+"FactoryOutStorageRecord/getFactoryOutStorageRecord","json","application/json;charset=utf-8",".factory-out-storage-record-table","threeDaysPage",otherday,today);
    })

    //近一周
    $("#fosr-aWeek").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-7*60*60*24*1000;
        initAjaxUseGet(path+"FactoryOutStorageRecord/getFactoryOutStorageRecord","json","application/json;charset=utf-8",".factory-out-storage-record-table","aWeekPage",otherday,today);
    })

    //近两周
    $("#fosr-twoWeeks").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-14*60*60*24*1000;
        initAjaxUseGet(path+"FactoryOutStorageRecord/getFactoryOutStorageRecord","json","application/json;charset=utf-8",".factory-out-storage-record-table","twoWeeksPage",otherday,today);
    })

    //近一个月
    $("#fosr-aMonth").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=DateToUnix(""+mydate.getFullYear()+"-"+mydate.getMonth()+"-"+mydate.getDate())
        initAjaxUseGet(path+"FactoryOutStorageRecord/getFactoryOutStorageRecord","json","application/json;charset=utf-8",".factory-out-storage-record-table","aMonthPage",otherday,today);
    })
})
