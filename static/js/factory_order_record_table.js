...
$(document).ready(function(){
    document.getElementById("for-start-time").valueAsDate=new Date();
    document.getElementById("for-end-time").valueAsDate=new Date();
    var targetJson;

    //初始页面
    var mydate=new Date();
    var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
    initAjaxUseGet(path+"WarehouseOrders","json","application/json;charset=utf-8",".factory-order-record-table","all",0,today);

    //筛选事件
    $("#for-select").on('click',function(){
        var $selectStartTime=DateToUnix($("#for-start-time").val());
        var $selectEndTime=DateToUnix($("#for-end-time").val());
        if($selectStartTime>$selectEndTime){
            alert("请选择正确的时间");
        }else{
            initAjaxUseGet(path+"WarehouseOrders","json","application/json;charset=utf-8",".factory-order-record-table","selectPage",$selectStartTime,$selectEndTime);
        }
    })

    //近三天
    $("#for-threeDays").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-3*60*60*24*1000;
        initAjaxUseGet(path+"WarehouseOrders","json","application/json;charset=utf-8",".factory-order-record-table","threeDaysPage",otherday,today);
    })

    //近一周
    $("#for-aWeek").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-7*60*60*24*1000;
        initAjaxUseGet(path+"WarehouseOrders","json","application/json;charset=utf-8",".factory-order-record-table","aWeekPage",otherday,today);
    })

    //近两周
    $("#for-twoWeeks").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-14*60*60*24*1000;
        initAjaxUseGet(path+"WarehouseOrders","json","application/json;charset=utf-8",".factory-order-record-table","twoWeeksPage",otherday,today);
    })

    //近一个月
    $("#for-aMonth").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=DateToUnix(""+mydate.getFullYear()+"-"+mydate.getMonth()+"-"+mydate.getDate())
        initAjaxUseGet(path+"WarehouseOrders","json","application/json;charset=utf-8",".factory-order-record-table","aMonthPage",otherday,today);
    })
})
