...
$(document).ready(function(){
    document.getElementById("fpis-start-time").valueAsDate=new Date();
    document.getElementById("fpis-end-time").valueAsDate=new Date();
    var targetJson;

    //初始页面
    var mydate=new Date();
    var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
    initAjaxUseGet(path+"statisticMaterialRecords","json","application/json;charset=utf-8",".material-purchase-statistics-table","all",0,today);

    //筛选事件
    $("#fpis-select").on('click',function(){
        var $selectStartTime=DateToUnix($("#fpis-start-time").val());
        var $selectEndTime=DateToUnix($("#fpis-end-time").val());
        if($selectStartTime>$selectEndTime){
            alert("请选择正确的时间");
        }else{
            initAjaxUseGet(path+"statisticMaterialRecords","json","application/json;charset=utf-8",".material-purchase-statistics-table","selectPage",$selectStartTime,$selectEndTime);
        }
    })
