...
$(document).ready(function(){
    document.getElementById("mpr-start-time").valueAsDate=new Date();
    document.getElementById("mpr-end-time").valueAsDate=new Date();
    var targetJson;

    //初始页面
    var mydate=new Date();
    var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
    initAjaxUseGet(path+"materialPurchaseRecords/searchResult","json","application/json;charset=utf-8",".material-purchase-record-table","all",0,today);

    //筛选事件
    $("#mpr-select").on('click',function(){
        var $selectStartTime=DateToUnix($("#mpr-start-time").val());
        var $selectEndTime=DateToUnix($("#mpr-end-time").val());
        if($selectStartTime>$selectEndTime){
            alert("请选择正确的时间");
        }else{
            initAjaxUseGet(path+"materialPurchaseRecords/searchResult","json","application/json;charset=utf-8",".material-purchase-record-table","selectPage",$selectStartTime,$selectEndTime);
        }
    })

    //近三天
    $("#mpr-threeDays").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-3*60*60*24*1000;
        initAjaxUseGet(path+"materialPurchaseRecords/searchResult","json","application/json;charset=utf-8",".material-purchase-record-table","threeDaysPage",otherday,today);
    })

    //近一周
    $("#mpr-aWeek").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-7*60*60*24*1000;
        initAjaxUseGet(path+"materialPurchaseRecords/searchResult","json","application/json;charset=utf-8",".material-purchase-record-table","aWeekPage",otherday,today);
    })

    //近两周
    $("#mpr-twoWeeks").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-14*60*60*24*1000;
        initAjaxUseGet(path+"materialPurchaseRecords/searchResult","json","application/json;charset=utf-8",".material-purchase-record-table","twoWeeksPage",otherday,today);
    })

    //近一个月
    $("#mpr-aMonth").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=DateToUnix(""+mydate.getFullYear()+"-"+mydate.getMonth()+"-"+mydate.getDate())
        initAjaxUseGet(path+"materialPurchaseRecords/searchResult","json","application/json;charset=utf-8",".material-purchase-record-table","aMonthPage",otherday,today);
    })
})
