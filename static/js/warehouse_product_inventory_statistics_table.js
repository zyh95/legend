...
$(document).ready(function(){
    document.getElementById("wpis-start-time").valueAsDate=new Date();
    document.getElementById("wpis-end-time").valueAsDate=new Date();
    var targetJson;
    ...
    initAjaxUseGet(path+"warehouseStatistics","json","application/json;charset=utf-8",".warehouse-product-inventory-statistics-table","all",0,today);

    //筛选事件
    $("#wpis-select").on('click',function(){
        var $selectStartTime=DateToUnix($("#wpis-start-time").val());
        var $selectEndTime=DateToUnix($("#wpis-end-time").val());
        if($selectStartTime>$selectEndTime){
            alert("请选择正确的时间");
        }else{
            initAjaxUseGet(path+"warehouseStatistics","json","application/json;charset=utf-8",".warehouse-product-inventory-statistics-table","selectPage",$selectStartTime,$selectEndTime);
        }
        
    })

    //近三天
    $("#wpis-threeDays").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-3*60*60*24*1000;
        initAjaxUseGet(path+"warehouseStatistics","json","application/json;charset=utf-8",".warehouse-product-inventory-statistics-table","threeDaysPage",otherday,today);
    })

    //近一周
    $("#wpis-aWeek").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-7*60*60*24*1000;
        initAjaxUseGet(path+"warehouseStatistics","json","application/json;charset=utf-8",".warehouse-product-inventory-statistics-table","aWeekPage",otherday,today);
    })

    //近两周
    $("#wpis-twoWeeks").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=today-14*60*60*24*1000;
        initAjaxUseGet(path+"warehouseStatistics","json","application/json;charset=utf-8",".warehouse-product-inventory-statistics-table","twoWeeksPage",otherday,today);
    })

    //近一个月
    $("#wpis-aMonth").on('click',function(){
        var mydate=new Date();
        var today=DateToUnix(""+mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate());
        var otherday=DateToUnix(""+mydate.getFullYear()+"-"+mydate.getMonth()+"-"+mydate.getDate())
        initAjaxUseGet(path+"warehouseStatistics","json","application/json;charset=utf-8",".warehouse-product-inventory-statistics-table","aMonthPage",otherday,today);
    })
})
...
function CreateTable(divName,targetId,_url,_startTime,_endTime){
    ...
    $(".table-content-modify").on('click',function(){
        var content=[];
        $(this).parent().find("td").each(function(index,elment){
            content.push($(elment).text());
        });
        $("#modal-in-storage-modify").modal("show")
            .on("shown.bs.modal",function(){
                var inputs = $("div#modal-in-storage-modify input.data-modify");//获取input元素，不包括select标签
                var selects = $("div#modal-in-storage-modify select.data-modify");
                var SELECT_AT_0=2,SELECT_AT_1=4;// 记录第2和4是select，其他是input
                $(selects[0]).val(content[SELECT_AT_0]);
                $(selects[1]).val(content[SELECT_AT_1]);   
                             
                for (let i = 0; i < inputs.length; i++) {
                    if(i < SELECT_AT_0){
                        $(inputs[i]).val(content[i]);
                    }else if(i == SELECT_AT_0){
                        $(inputs[i]).val(content[i+1]);
                    }else{
                        $(inputs[i]).val(content[i+2]);
                    }
                }
            });    
    });    
